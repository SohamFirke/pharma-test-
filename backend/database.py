...
class Database:
    ...
    # ========== Prescription Database Methods ==========
    
    @staticmethod
    def save_prescription(prescription_data: Dict) -> bool:
        """Save prescription record."""
        try:
            prescriptions_path = Path(__file__).parent.parent / "data" / "prescriptions.csv"
            ...
            if prescriptions_path.exists():
                ...
                df = pd.DataFrame(columns=[
                    'prescription_id', 'user_id', 'file_path', 'upload_date',
                    'status', 'ocr_text', 'parsed_medicines', 'safety_status',
                    'order_ids', 'trace_id'
                ])
            
            # Add new prescription
            new_row = pd.DataFrame([prescription_data])
            df = pd.concat([df, new_row], ignore_index=True)
            
            df.to_csv(prescriptions_path, index=False)
            return True
    ...
    def get_prescription(prescription_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            df = pd.read_csv(prescriptions_path)
            result = df[df['prescription_id'] == prescription_id]
            if result.empty:
                return None
    ...
    def update_prescription_status(prescription_id: str, status: str,
                                  order_ids: str = None) -> bool:
        """Update prescription status and order IDs."""
        try:
            prescriptions_path = Path(__file__).parent.parent / "data" / "prescriptions.csv"
            if not prescriptions_path.exists():
                return False
            ...
            df = pd.read_csv(prescriptions_path)
            mask = df['prescription_id'] == prescription_id
            ...
            if order_ids:
                df.loc[mask, 'order_ids'] = order_ids
    ...
    @staticmethod
    def get_all_prescriptions(user_id: str = None) -> List[Dict]:
