...
- quantity: Number of units requested (tablets, capsules, ml, etc.) - MUST be an integer number. If user says "20 tablets", quantity is 20. If user says "3 tablets", quantity is 3.
- dosage_per_day: How many units per day (default to 1 if not specified)

CRITICAL: Extract the EXACT NUMBER mentioned for quantity. Examples:
- "20 tablets of paracetamol" -> quantity: 20
- "3 tablets" -> quantity: 3  
- "I want paracetamol 50 units" -> quantity: 50
- "100 capsules" -> quantity: 100
...
{{"medicine_name": "...", "quantity": <number>, "dosage_per_day": <number>}}"""
        ...
        try:
            ...
            if json_match:
                ...
                # Ensure dosage_per_day is an integer
                if 'dosage_per_day' in result:
                    try:
                        result['dosage_per_day'] = int(result['dosage_per_day'])
                    except (ValueError, TypeError):
                        result['dosage_per_day'] = 1
                else:
                    result['dosage_per_day'] = 1
                ...
                # Debug logging
                print(f"[Ollama Extract] Message: '{message}' -> quantity: {result.get('quantity')}, medicine: {result.get('medicine_name')}")
